...
class FilterQuizViewModel: ViewModel() {
    val selectedCategory = MutableLiveData<Int?>(null)
    val selectedDifficulty = MutableLiveData<String?>(null)

    val isFormValid = MediatorLiveData<Boolean>().apply {
        addSource(selectedCategory) { updateValidity() }
        addSource(selectedDifficulty) { updateValidity() }
    }

    private fun updateValidity() {
        val categorySelected = selectedCategory.value != null
        val difficultySelected = selectedDifficulty.value != null
        isFormValid.postValue(categorySelected && difficultySelected)
    }
